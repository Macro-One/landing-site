...
export const Navbar: React.FC = () => {
  ...
  const navLinks: NavLink[] = [
    ...
    { name: 'AI Infrastructure', path: '/ai-infrastructure' },
  ];
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center space-x-8">
            ...
            if (link.path) {
              return (
                <Link
                  key={link.name}
                  href={link.path}
                  className={`text-sm font-medium transition-colors uppercase tracking-widest ${pathname === link.path ? 'text-macro-orange' : 'text-neutral-400 hover:text-macro-orange'
                    }`}
                >
                  {link.name}
                </Link>
          ...
          <button
            ...
            className="px-6 py-2 bg-macro-orange text-white font-bold rounded-full transition-all hover:bg-orange-600 hover:scale-105 cursor-pointer"
          >
            Get Started
          </button>
        </div>
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-macro-charcoal border-b border-white/10 p-6 flex flex-col space-y-4 shadow-2xl max-h-[80vh] overflow-y-auto">
          {navLinks.map((link) => {
            if (link.dropdown) {
              return (
            ...
            if (link.path) {
              return (
                <Link
                  key={link.name}
                  href={link.path}
                  className="text-lg font-medium text-neutral-400 hover:text-macro-orange"
                  onClick={() => setIsMobileMenuOpen(false)}
                >
                  {link.name}
                </Link>
